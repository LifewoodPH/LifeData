  ...
  try {
    ...
    if (req.method === 'POST') {
        try {
            ...
            requestedPath = body.path || (body.country && body.country.includes('.xls') ? body.country : null);
        } catch (e) {}
    }
    ...
    if (isExcel) {
        ...
        // Use 'dense' mode for significantly better memory efficiency
        const workbook = XLSX.read(buffer, { 
            type: 'array',
            dense: true,
            sheetRows: 500, // Reduced to 500 rows for absolute stability on large files
            cellStyles: false,
            cellFormula: false,
            cellDates: true,
            cellNF: false,
        ...
        // MANUALLY parse dense worksheet to avoid 'sheet_to_json' memory spike
        // In dense mode, data is in worksheet['!data'][row][col]
        const denseData = (worksheet as any)['!data'] || [];
        rows = denseData.map((row: any[]) => {
            if (!row) return [];
            return row.map(cell => (cell && cell.v !== undefined) ? cell.v : "");
        });
    } else {
    ...
    for (let i = 0; i < Math.min(rows.length, 10); i++) {
        ...
        if (rowData && rowData.some && rowData.some((c: any) => String(c).toLowerCase().includes('first name') || String(c).toLowerCase().includes('email address'))) {
            headerRowIdx = i;
            break;
        }
    }
    ...
    if (headerRowIdx !== -1) {
        headers = rows[headerRowIdx].map((h: any) => String(h || '').trim());
